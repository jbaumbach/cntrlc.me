...
exports.start = function(server, cb) {
  ...
  //
  // Use "detect_buffers" to prevent a specific type of error when using redis for socket.io communication
  //
  redis.createClient({ detect_buffers: true }, function(err, sub) {
    if (err) {
      cb(err);
    } else {
      var pub = redis.getClient();
